...
// Charge Crisp UNE SEULE FOIS si activé
window.loadCrispIfEnabled = function () {
  const cfg = window.APP_CONFIG?.chat;
  if (cfg?.provider === "crisp" && cfg?.siteId) {
    if (document.querySelector('script[src*="client.crisp.chat"]')) return; // anti-doublon
...
// Installe App Check reCAPTCHA v3 si fourni
